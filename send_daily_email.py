...
def send(image_path: Path, recipients: list[str], date_str: str) -> None:
    ...
    msg["Subject"] = f"인천공항 국제선 출발편 현황 ({date_str})"
    msg["From"] = formataddr(("인천공항 출발편 자동발송", user))
...
<html><body style="font-family:'Noto Sans KR','Helvetica Neue',Arial,sans-serif;color:#222;background:#f7f7fa;padding:20px;margin:0;">
  <div style="max-width:960px;margin:0 auto;background:#fff;padding:28px;border-radius:8px;border:1px solid #e5e5ea;">
    ...
    <p style="margin:0 0 20px 0;font-size:14px;color:#444;"><strong>인천공항 국제선 출발편 현황</strong>을 공유드립니다.</p>
    <p style="margin:0 0 20px 0;">
      <img src="cid:dashboard" alt="인천공항 국제선 출발편 현황 {date_str}" style="max-width:100%;height:auto;display:block;border:1px solid #ddd;border-radius:4px;">
    </p>
...
def main() -> int:
    parser = argparse.ArgumentParser(description="인천공항 출발편 대시보드 일일 메일링")
